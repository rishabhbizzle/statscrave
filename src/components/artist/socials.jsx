...
import { FaSpotify, FaInstagram, FaTiktok, FaYoutube, FaLine, FaRadio, FaMusic  } from 'react-icons/fa';
...
const ArtistSocials = ({ id }) => {
  ...
  const fetchArtistSocialsData = async () => {
    ...
    try {
      ...
      toast.error(error?.message || "An error occurred while fetching artist socials data.");
      console.error(error);
    }
    setLoading(false);
  };
  ...
  return (
    <div>
        {socialData?.socialSummary && (
            ...
            <h1 className="text-2xl font-bold mb-4">Platforms Statistics</h1>
