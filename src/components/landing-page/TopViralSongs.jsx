...
const TopViralSongs = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_BASE_URL}/songs/viral?limit=10`
      );
  ...
  if (loading) {
    return (
      <Card className="w-full">
        <CardHeader>
          <CardTitle>Top Viral Songs Currently</CardTitle>
          <CardDescription>
            Here are the top 10 viral songs globally right now.
          </CardDescription>
        </CardHeader>
        <CardContent>
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        <CardTitle className="text-xl lg:text-2xl">Top Viral Songs Currently</CardTitle>
        <CardDescription>
          Here are the top 10 viral songs globally right now.
        </CardDescription>
      </CardHeader>
